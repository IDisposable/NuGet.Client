...
namespace NuGet.Protocol
{
    public class MetadataResourceV2Feed : MetadataResource
    {
        ...
        public MetadataResourceV2Feed(HttpSourceResource httpSourceResource, Configuration.PackageSource packageSource)
        {
        ...
        public override async Task<IEnumerable<NuGetVersion>> GetVersions(string packageId, bool includePrerelease, bool includeUnlisted, ILogger log, CancellationToken token)
        {
            ...
            try
            {
                var packages = await _feedParser.FindPackagesByIdAsync(packageId, null, includeUnlisted, includePrerelease, log, token);
